...
export default function ModernTemplate({ data, primaryColor, font }: CVTemplateProps) {
  const gradientStyle = {
    ...
    <div style={{ fontFamily: font }} className="min-h-screen bg-white">
      ...
      <div className="relative overflow-hidden" style={gradientStyle}>
        ...
        <div className="relative z-10 text-center py-20 px-8">
          <motion.div
            ...
            className="w-24 h-24 rounded-full mx-auto mb-6 flex items-center justify-center text-4xl font-bold text-white shadow-2xl"
      ...
      <div className="max-w-4xl mx-auto px-8 py-12 space-y-12">
        ...
        {data?.personalInfo?.bio && (
          <RevealSection>
            <div className="p-6 rounded-2xl border-l-4" style={{ borderColor: primaryColor, backgroundColor: primaryColor + '08' }}>
              <p className="text-slate-600 leading-relaxed italic">{data.personalInfo.bio}</p>
            </div>
          </RevealSection>
        )}
        ...
        {data?.experience?.length > 0 && (
          <RevealSection delay={0.1}>
            <h2 className="text-2xl font-bold mb-6 pb-2 border-b-2" style={{ color: primaryColor, borderColor: primaryColor }}>
            ...
            <div className="space-y-6">
              {data.experience.map((exp: any, idx: number) => (
                <motion.div key={idx}
                  ...
                  className="relative pl-6 border-l-2" style={{ borderColor: primaryColor + '40' }}>
                  <div className="absolute -left-2 top-0 w-4 h-4 rounded-full" style={{ backgroundColor: primaryColor }} />
                  <div className="flex items-start justify-between mb-1">
                    <h3 className="font-bold text-slate-800 text-lg">{exp.position}</h3>
                    <span className="text-sm text-slate-400 ml-4 flex-shrink-0">{exp.startDate} — {exp.endDate || 'Actualidad'}</span>
                  </div>
                  <p className="font-medium mb-2" style={{ color: primaryColor }}>{exp.company}</p>
                  {exp.description && <p className="text-slate-600 text-sm leading-relaxed mb-3">{exp.description}</p>}
                  {exp.achievements?.length > 0 && (
                    <ul className="space-y-1">
                      {exp.achievements.map((a: string, i: number) => (
                        <li key={i} className="flex items-start gap-2 text-slate-600 text-sm">
                          <span style={{ color: primaryColor }} className="mt-1">▸</span> {a}
                        </li>
                      ))}
                    </ul>
                  )}
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {data.education.map((edu: any, idx: number) => (
                <motion.div key={idx}
                  ...
                  {edu.field && <p className="text-sm text-slate-500">en {edu.field}</p>}
          ...
          <RevealSection delay={0.3}>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {data.skills.technical?.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-4">Técnicas</h3>
                  {data.skills.technical.map((s: string, i: number) => (
                    <SkillBar key={i} skill={s} color={primaryColor} delay={i * 0.1} />
                  ))}
                </div>
              )}
            ...
            {data.skills.tools?.length > 0 && (
              <div className="mt-6">
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Herramientas</h3>
                <div className="flex flex-wrap gap-2">
                  {data.skills.tools.map((s: string, i: number) => (
                    <motion.span key={i}
                      ...
                      style={{ backgroundColor: primaryColor + '15', color: primaryColor, border: `1px solid ${primaryColor}40` }}>
                      {s}
                    </motion.span>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {data.languages.map((l: any, idx: number) => (
                <motion.div key={idx}
                  ...
                  <p className="text-sm mt-1" style={{ color: primaryColor }}>{l.level}</p>
                </motion.div>
              ))}
            </div>
          </RevealSection>
              ...
              {data.certifications.map((c: any, idx: number) => (
                <motion.div key={idx}
                  ...
                  <div>
                    ...
                    {c.issuer && <p className="text-sm text-slate-500">{c.issuer}{c.date ? ` · ${c.date}` : ''}</p>}
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <RevealSection delay={0.5}>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {data.projects.map((p: any, idx: number) => (
                <motion.div key={idx}
                  whileHover={{ scale: 1.02, boxShadow: `0 8px 30px ${primaryColor}20` }}
